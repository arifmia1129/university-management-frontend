...
const updateSemesterRegistration = ({ params }: { params: any }) => {
  const { data: semesterRegistration } = useGetSemesterRegistrationByIdQuery(
    params.id
  );

  const availableStatusOptions = semesterRegistrationStatusOptions.map(
    (status: any) => {
      ...
      } else if (semesterRegistration?.status === "ENDED") {
        if (status.value === "ONGOING" || status.value === "UPCOMING") {
          status.disabled = true;
        }
      }
  ...
  return (
    <div>
      ...
      <Form
        ...
        defaultValues={semesterRegistration}
      >
        <div
          style={{
