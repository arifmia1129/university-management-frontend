...
import UMCollapse, { Item } from "@/components/ui/UMCollapse";
import {
...
const ViewPreregistrationPage = () => {
  const { data, isLoading } = useGetStudentSemesterRegistrationQuery({});
  const [enrollIntoCourse] = useEnrollSemesterRegistrationMutation();
  const [withdrawFromCourse] = useWithdrewSemesterRegistrationMutation();

  const [confirmSemesterRegistration] =
    useConfirmSemesterRegistrationMutation();
  ...
  const handleConfirmRegistration = async () => {
    message.loading("Registration processing...");
    try {
      const res = await confirmSemesterRegistration({}).unwrap();
      if (res) {
  ...
  const availableCourses: Item[] = data?.semesterRegistration?.map(
  ...
  const isAtLeastOneCourseTaken =
    availableCourses?.filter((el: Item) => el.isTaken === true).length > 0
      ? true
      : false;
  ...
  return (
    <>
      ...
      <UMCollapse items={availableCourses} onChange={onChange} />
